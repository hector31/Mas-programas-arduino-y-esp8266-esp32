...
//      int valor=str.toInt();//IMPRIME DATOS
//      //tiempo = map(valor,0,180,8000,10); 
//      Serial.println(tiempo);
//      //In my case I've used valor = map(Serial.read(),0,255,7000,10); for better results
//   }
if(Serial.available()>0)
   { 
      tiempo = map(Serial.read(),0,255,10000,10); 
      //In my case I've used valor = map(Serial.read(),0,255,7000,10); for better results
   }
    if (detectado)
    {
      Serial.println("Entro");
      delayMicroseconds(tiempo); //This delay controls the power
      digitalWrite(3,HIGH);
